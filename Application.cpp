...
HRESULT Application::Initialise(HINSTANCE hInstance, int nCmdShow)
{
	...
	XMFLOAT3 position = { 0.0f, 0.0f, -10.f };
...
HRESULT Application::InitDevice()
{
	...
	solidShapeDesc.CullMode = D3D11_CULL_BACK;
	...
	CreateDDSTextureFromFile(_pd3dDevice, L"Crate_COLOR.dds", nullptr, &_pShaderRV);
	//CreateDDSTextureFromFile(_pd3dDevice, L"Crate_NRM.dds", nullptr, &_pShaderRV);
...
void Application::Cleanup()
{
	...
	if (_pShaderRV) _pShaderRV->Release();
}
void Application::Update()
{
    ...
    // Animate the cube
    //
	XMStoreFloat4x4(&_worldArray[0], XMMatrixRotationY(t));
	XMStoreFloat4x4(&_worldArray[1], XMMatrixTranslation(5, 0, 0) * XMMatrixScaling(0.5, 0.5, 0.5) * XMMatrixRotationY(t * 2));
	XMStoreFloat4x4(&_worldArray[2], XMMatrixRotationY(t * 5) * XMMatrixTranslation(10, 0, 0) * XMMatrixScaling(0.75, 0.75, 0.75) * XMMatrixRotationY(t * 0.75));
	XMStoreFloat4x4(&_worldArray[3], XMMatrixTranslation(10, 0, 0) * XMMatrixScaling(0.25, 0.25, 0.25) * XMMatrixRotationZ(t * 6) * XMMatrixTranslation(9, 0, 0) * XMMatrixRotationY(t*0.75));
	XMStoreFloat4x4(&_worldArray[4], XMMatrixTranslation(10, 0, 0) * XMMatrixScaling(0.25, 0.25, 0.25) * XMMatrixRotationY(t * 4) * XMMatrixTranslation(9, 0, 0) * XMMatrixRotationY(t*0.75));
...
void Application::Draw()
{
	...
	_pImmediateContext->PSSetShaderResources(0, 1, &_pShaderRV);
	...
	cb.mWorld = XMMatrixTranspose(worldArr[0]);
	...
	cb.mWorld = XMMatrixTranspose(worldArr[1]);
	_pImmediateContext->UpdateSubresource(_pConstantBuffer, 0, nullptr, &cb, 0, 0);
	_pImmediateContext->DrawIndexed(36, 0, 0);

	cb.mWorld = XMMatrixTranspose(worldArr[2]);
	_pImmediateContext->UpdateSubresource(_pConstantBuffer, 0, nullptr, &cb, 0, 0);
	_pImmediateContext->DrawIndexed(36, 0, 0);

	cb.mWorld = XMMatrixTranspose(worldArr[3]);
	_pImmediateContext->UpdateSubresource(_pConstantBuffer, 0, nullptr, &cb, 0, 0);
	_pImmediateContext->DrawIndexed(36, 0, 0);

	cb.mWorld = XMMatrixTranspose(worldArr[4]);
	_pImmediateContext->UpdateSubresource(_pConstantBuffer, 0, nullptr, &cb, 0, 0);
	_pImmediateContext->DrawIndexed(36, 0, 0);
